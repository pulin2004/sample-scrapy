...
class Trend():
    def __init__(self, nextPoint,max_dif = 0.1 ,min_dif=0.05,boll_col = 1,close_col = 4):
        ...
        self.__max_dif = max_dif
        self.__boll_col = boll_col
        self.__close = close_col
        self.__min_dif = min_dif
...
class DownTrend(Trend):
    ...
    def getTrendEnum(self, boll):
        ...
        _firstValus = _first.getColumnAverage(self.__boll_col)
        _lastValus = _last.getColumnAverage(self.__boll_col)
        if float((_firstValus-_lastValus)/_lastValus) >=self.__max_dif :
            boll.setMsg("下降振幅，开始%s-结束%s，超过%s"%[_firstValus,_lastValus,self.__max_dif])
            return Numbers.DOWN
        else:
            ...
            for _data in boll.getData():
                if _data.isSection(_sec):
                    if (_data.getColumnAverage(self.__boll_col) - _data.getColumnAverage(self.__close))<0:
                        _mark = False
                        break
            if _mark:
                ...
                return self._next.getTrendEnum()
...
class DownFlatTrend(Trend):
    ...
    def getTrendEnum(self, boll):
        ...
        _firstValus = _first.getColumnAverage(self.__boll_col)
        _lastValus = _last.getColumnAverage(self.__boll_col)
        if float((_firstValus - _lastValus) / _lastValus) >= self.__min_dif:
            _sec = boll.getHighSections();
            for _data in boll.getData():
                if _data.isSection(_sec):
                    if (_data.getColumnAverage(self.__boll_col) - _data.getColumnAverage(self.__close)) > 0:
...
class UpHighTrend(Trend):
    ...
    def getTrendEnum(self, boll):
        ...
        _firstValus = _first.getColumnAverage(self.__boll_col)
        _lastValus = _last.getColumnAverage(self.__boll_col)
        if float((_lastValus-_firstValus)/_firstValus) >=self.__max_dif :
            _sec = boll.getHighSections();
            _mark = True
            for _data in boll.getData():
                if _data.isSection(_sec):
...
def manageTrend():
    return DownTrend(DownFlatTrend(UpHighTrend(FlatUpTrend())))
