...
#获取股票列表
#code,代码 name,名称 industry,所属行业 area,地区 pe,市盈率 outstanding,流通股本totals,总股本(万) totalAssets,总资产(万)liquidAssets,流动资产
# fixedAssets,固定资产 reserved,公积金 reservedPerShare,每股公积金 eps,每股收益bvps,每股净资 pb,市净率 timeToMarket,上市日期
def Get_Stock_List():
...
def Get_TA(df_Code,Dist):
    ...
    for code in df_Code.index:
        print('start stock code:'+code)
    # index,0 - 6 date：日期 open：开盘价 high：最高价 close：收盘价 low：最低价volume：成交量 price_change：价格变动 p_change：涨跌幅
    # 7-12 ma5：5日均价 ma10：10日均价 ma20:20日均价 v_ma5:5日均量v_ma10:10日均量v_ma20:20日均量
        df = ts.get_hist_data(code, start='2016-11-01')
        try:
            dflen = df.shape[0]
            count = count + 1
            if dflen > 35:
...
def Get_MACD(df):
    ...
    MAlen = len(SignalMA5)
    operate = 0
    # 2个数组1.DIFF、DEA均为正，DIFF向上突破DEA，买入信号。2.DIFF、DEA均为负，DIFF向下跌破DEA，卖出信号。
    # 待修改
...
def Output_Csv(df, Dist):
    ...
    df.to_csv(Dist + CURRENTDAY + 'stock1.csv', encoding='gbk')  # 选择保存 
...
print('start this stock.py!')
df = Get_Stock_List()
Dist = '/Users/pulin/Documents/temp/stock/'
print('one this stock.py!')
df = Get_TA(df, Dist)
Output_Csv(df, Dist)
print('end this stock.py!')
time.sleep(1)
Close_machine()
print('close this stock.py!')